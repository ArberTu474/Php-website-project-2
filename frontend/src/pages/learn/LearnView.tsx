...
export default function LearnView() {
  ...
  if (!course) {
    return (
      <div className="flex h-[calc(100dvh-65px)] items-center justify-center overflow-hidden">
        <div className="mx-auto my-0 size-16 animate-spin rounded-full border-6 border-border border-t-transparent" />
      </div>
    )
  }
  ...
  return (
    <div className="flex h-[calc(100dvh-65px)] overflow-hidden">
      <aside
        className={`flex flex-col overflow-hidden border-r border-border bg-background transition-all duration-200 ease-in-out ${
          sidebarOpen ? "w-75 min-w-75" : "w-0 min-w-0"
        }`}
      >
        <div className="pt-2 pb-4">
          <div className="px-4 pb-2">
            <Button className="p-0" variant="link" asChild>
              <Link to={`/courses/${slug}`} className="flex items-center gap-0 leading-none">
                <ChevronLeft size={16} />
            ...
            <div className="px-4">
              <Progress
                ...
                className={`mb-1 h-1.5 ${
                  enrollment.progress.percent === 100 ? "[&>div]:bg-green-600/60" : ""
                }`}
              />
              <p className="text-xs font-semibold">
                {enrollment.progress.completed_lessons}/
                {enrollment.progress.total_lessons} ({enrollment.progress.percent}%)
              </p>
            </div>
          )}
        ...
        <div className="flex-1 overflow-auto">
          {course.modules?.map((module: Module) => (
            <div key={module.module_id}>
              <div className="border-b border-border bg-muted px-4 py-1 text-sm font-semibold uppercase">
                {module.title}
              </div>
              ...
              {module.lessons.map((l: Lesson) => {
                ...
                const isDone =
                  enrollment?.completed_lesson_ids?.includes(l.lesson_id) ?? false

                return (
                  <button
                    key={l.lesson_id}
                    onClick={() => goToLesson(l)}
                    className={`flex w-full cursor-pointer items-start gap-1 border-b border-border px-4 py-2 text-left ${
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p
                        ...
                        } ${isDone && !isActive ? "italic text-muted-foreground" : ""}`}
                      >
                        {l.title}
                      </p>
      ...
      <div className="flex flex-1 flex-col overflow-auto">
        ...
        <div className="mx-auto my-0 w-full max-w-[1000px] p-4">
          {lessonLoading ? (
            <div className="my-10">
              <div className="mx-auto my-0 size-16 animate-spin rounded-full border-6 border-border border-t-transparent" />
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {lesson.duration_mins && (
                  <Badge variant="secondary" className="text-muted-foreground">
                    <Clock size={14} />
                    <span className="ml-1">{lesson.duration_mins} min</span>
                  </Badge>
                )}
              </div>
