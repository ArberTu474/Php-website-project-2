...
export default function CourseDetailPage() {
  ...
  const isEnrolled =
    myEnrollments?.some((e) => e.course.course_id === course?.course_id) ?? false

  const enrollment =
    myEnrollments?.find((e) => e.course.course_id === course?.course_id) ?? null

  const existingReview =
    course?.reviews?.find((r) => r.student_id === user?.user_id) ?? null
  ...
  if (isError || !course) {
    return (
      <div className="mt-12 flex flex-col items-center gap-2">
        <TriangleAlert size={64} className="fill-destructive/15 stroke-destructive" />
  ...
  function renderEnrollButton() {
    ...
    if (isEnrolled) {
      return (
        ...
        >
          <Check size={16} />
          <span className="ml-1">Already enrolled</span>
        </Button>
      )
    }
  ...
  return (
    <div className="container mx-auto my-0 px-6 py-3">
      <div>
        ...
        <p className="max-w-[90ch] text-muted-foreground">{course.description}</p>
          ...
          <div className="flex flex-col gap-2 text-sm leading-none text-muted-foreground sm:flex-row md:gap-4">
            ...
            {course.avg_rating && (
              <span className="flex items-center gap-0.5 text-sm leading-none font-semibold text-yellow-400/80">
                <Star className="fill-yellow-400/80 stroke-0" size={16} />
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          {course.avg_rating && (
            <span className="flex items-center gap-1 text-base font-semibold text-yellow-400/80">
              ...
              <span>{course.avg_rating}</span>
              <span className="font-normal">({course.total_reviews})</span>
            </span>
          )}
        </div>
