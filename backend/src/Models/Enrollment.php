...
class Enrollment
{
  ...
  public static function findByStudent(string $studentId): array
  {
    ...
    $stmt = $db->prepare('
        ...
        GROUP BY e.enrollment_id, e.enrolled_at, e.completed_at,
                 c.course_id, c.title, c.slug,
                 c.thumbnail_url, c.description,
    ...
    // Prepared once, reused per enrollment row
    ...
    return array_map(function (array $row) use ($progressStmt): array {
      ...
      $completedLessonIds = array_map('strval', $progressStmt->fetchAll(\PDO::FETCH_COLUMN));
